...
- **Frontend:** `/apex` (list + region/status filters), `/apex/:id` (ranked results, rows expand to per-game tables — rows are clickable divs, not buttons, because they contain the follow-star button and nested buttons are invalid HTML), "ALGS" nav item, Apex-specific team page layout (recent day placements instead of the LoL roster/matches sections, which would read as permanently broken-empty).
...
- 182 backend tests total (up from 154): `CitoSyncServiceTest` (same pure-function/orchestration split as `RiotSyncServiceTest`, plus `deriveDayStatus` staleness cases — see Post-launch fixes below), `ApexMatchDayServiceTest`, `ApexMatchDayControllerTest`, plus guard tests in `TeamServiceTest`. Verified end-to-end against the real Cito API: 50 real ALGS events synced (Year 6 Pro League + EWC Playoffs), 120 teams, 720 team results, 4320 game results; browse/detail/follow/feed/search all exercised in the browser.

**Post-launch fixes (2026-07-12) — done.** Found by the user actually using the feature:
- **Stale ALGS status:** Cito sometimes leaves a long-past match day marked `pending` for days after it was actually played (observed live: the Year 6 EWC Playoffs group-stage days were still `pending` three days later). `CitoSyncService.deriveDayStatus` now treats a `pending` day whose start time is more than 12h in the past as `FINISHED` regardless of what Cito's status field says — an ALGS day runs its ~6 games in a few hours, so "pending" past that window can only mean Cito is lagging, not that the day hasn't happened.
- **Feed no longer splits by game:** the separate "Upcoming ALGS match days" section was replaced with one combined, time-sorted "Upcoming matches" list mixing `MatchResponse` and `ApexMatchDayResponse` items — a user following multiple games doesn't want their feed partitioned by title. `LeagueChip` (small colored per-game tag, e.g. "LEC"/"ALGS EMEA") and `GameIcon` (per-game logo/emoji, see below) are what identify the game per card now, not separate sections. Match/match-day cards dropped the full tournament name from their header row too — it was long enough on some ALGS tournaments to visually overlap the centered countdown; the league chip alone is enough context, full tournament name is still on the detail page.
- **Game icons:** `GameIcon` looks for a local file at `frontend/public/logos/{gameSlug}.png` and falls back to an emoji (⚔️ LoL, 🐲 Dota 2, 🪂 Apex) if the file 404s/fails to decode — never a broken-image icon. Bundled as local files rather than hotlinked: confirmed by direct probing that no publisher hosts a hotlinkable "game logo" asset — Data Dragon (already used here for champion icons) only serves champion/item/profile-icon assets, EA/Respawn has no public asset CDN at all, and Valve's Steam CDN has a real Dota 2 image but only as a wide promotional banner, not a square icon. LoL and Apex logos are in the repo (`frontend/public/logos/`, sourced by the user directly — worth a quick check against each publisher's brand guidelines before treating this as settled for a public repo); Dota still falls back to emoji.
- **Match/match-day card layout:** both card types are `flex flex-col` with a `flex-1` middle section (team row for `MatchCard`, day-name for `ApexMatchDayCard`) so that when the two card types sit in the same stretched grid row (e.g. the feed, which now mixes both), whichever card is naturally shorter has its content properly centered instead of leaving dead space, and the `GameIcon` at the bottom always lands at the same distance from the card's true bottom edge regardless of which card type is taller. `useCountdown`/`formatCountdown` were extracted from `MatchCard` into a shared `frontend/src/components/useCountdown.ts` so `ApexMatchDayCard` gets the identical top-center countdown (previously missing entirely on ALGS cards).
- **Stale-status refetch:** `useMatches`/`useFeed`/`useApexMatchDays`/`useUpcoming` now poll every 60s via `refetchInterval` (only while the tab is focused — TanStack Query's `refetchIntervalInBackground` default). Without this, a match's countdown could hit "Starting now" and just sit there indefinitely on an open tab, since the backend only flips `UPCOMING → RUNNING` on its own sync tick and TanStack Query otherwise only refetches on refocus/remount.
...
- 182 tests total as of 2026-07-12 (up from 153 on 2026-07-10; Apex/Cito coverage plus the `deriveDayStatus` staleness fix)
...
- **Apex/ALGS:** "ALGS" nav item → `ApexPage` (`/apex`, match day list with region/status filters) and `ApexMatchDayDetailPage` (`/apex/:id`, ranked 20-team results; each row expands to a per-game placement/kills/points table — rows are clickable divs, not `<button>`s, because they embed the follow-star button and nested buttons are invalid HTML). The feed's "Upcoming matches" list mixes `MatchResponse` and `ApexMatchDayResponse` items sorted by time rather than a separate ALGS section — `LeagueChip`/`GameIcon` identify the game per card (see Post-launch fixes). Apex team pages replace the LoL roster/matches sections with "Recent ALGS results" (Cito has no roster or head-to-head data). Settings league picker works for Apex unchanged (ALGS regions are Leagues).
